/** @file OctomapDoseServer.cpp
 * @brief Dose octomap server implementation
 *  Modified from OctomapServer.cpp
 * 
...
OctomapDoseServer::OctomapDoseServer(const ros::NodeHandle private_nh_, const ros::NodeHandle &nh_): 
  m_nh(nh_),
  ...
  double temp;
  m_nh_private.param("uv_power", temp, 480.0);
  m_power = temp;
  
  m_nh_private.param("uv_timestep", temp, 0.01);
  m_timeStep = temp;

  m_nh_private.param("uv_dose_threshold", temp, 1000.0);
  m_doseThreshold = temp;
  ...
  m_tfPointCloudSub->registerCallback(boost::bind(&OctomapDoseServer::insertCloudCallback, this, _1));

  m_octomapBinaryService = m_nh.advertiseService("octomap_binary", &OctomapDoseServer::octomapBinarySrv, this);
  m_octomapFullService = m_nh.advertiseService("octomap_full", &OctomapDoseServer::octomapFullSrv, this);
  m_clearBBXService = m_nh_private.advertiseService("clear_bbx", &OctomapDoseServer::clearBBXSrv, this);
  m_resetService = m_nh_private.advertiseService("reset", &OctomapDoseServer::resetSrv, this);

  dynamic_reconfigure::Server<OctomapServerConfig>::CallbackType f;
  f = boost::bind(&OctomapDoseServer::reconfigureCallback, this, _1, _2);
  m_reconfigureServer.setCallback(f);
}

OctomapDoseServer::~OctomapDoseServer() {
...
bool OctomapDoseServer::openFile(const std::string &filename) {
...
void OctomapDoseServer::insertCloudCallback(const sensor_msgs::PointCloud2::ConstPtr &cloud) {
...
void OctomapDoseServer::insertScan(const tf::Point &sensorOriginTf, const PCLPointCloud &ground, const PCLPointCloud &nonground) {
  ...
  for (KeySet::iterator it = occupied_cells.begin(), end = occupied_cells.end(); it != end; it++) {
    ...
    float dose = m_power * m_timeStep / (4 * M_PI * pow((m_octree->keyToCoord(*it) - sensorOrigin).norm(), 2));
    m_octree->incrementNodeDose(*it, dose);
  }
...
void OctomapDoseServer::publishAll(const ros::Time &rostime) {
  ros::WallTime startTime = ros::WallTime::now();
  ...
  for (OcTreeT::iterator it = m_octree->begin(m_maxTreeDepth),
    ...
    if (m_octree->isNodeOccupied(*it)) {
      ...
      if (z + half_size > m_occupancyMinZ && z - half_size < m_occupancyMaxZ) {
        ...
        float dose_norm = dose / m_doseThreshold;
        ...
        if (publishMarkerArray) {
          ...
          if (m_useColoredMap) {
            if (dose_norm > 1.0) {
              ...
              tinycolormap::Color color_converter = tinycolormap::GetColor(dose_norm, tinycolormap::ColormapType::Viridis);
              
              std_msgs::ColorRGBA _color; 
              _color.r = color_converter.r(); 
              _color.g = color_converter.g(); 
              _color.b = color_converter.b(); 
              _color.a = 1.0;
              occupiedNodesVis.markers[idx].colors.push_back(_color);
            }
          }
  ...
  double total_elapsed = (ros::WallTime::now() - startTime).toSec();
  ROS_DEBUG("Map publishing in OctomapDoseServer took %f sec", total_elapsed);
}

bool OctomapDoseServer::octomapBinarySrv(OctomapSrv::Request &req,
...
bool OctomapDoseServer::octomapFullSrv(OctomapSrv::Request &req,
  OctomapSrv::Response &res) {
...
bool OctomapDoseServer::clearBBXSrv(BBXSrv::Request &req, BBXSrv::Response &resp) {
...
bool OctomapDoseServer::resetSrv(std_srvs::Empty::Request &req, std_srvs::Empty::Response &resp) {
  visualization_msgs::MarkerArray occupiedNodesVis;
  occupiedNodesVis.markers.resize(m_treeDepth + 1);
...
void OctomapDoseServer::publishBinaryOctoMap(const ros::Time &rostime) const {
  Octomap map;
...
void OctomapDoseServer::publishFullOctoMap(const ros::Time &rostime) const {
  Octomap map;
  map.header.frame_id = m_worldFrameId;
  map.header.stamp = rostime;
...
void OctomapDoseServer::filterGroundPlane(const PCLPointCloud &pc, PCLPointCloud &ground, PCLPointCloud &nonground) const {
  ground.header = pc.header;
  nonground.header = pc.header;

  if (pc.size() < 50) {
    ROS_WARN("Pointcloud in OctomapDoseServer too small, skipping ground plane extraction");
    nonground = pc;
  }
  else {
...
void OctomapDoseServer::handlePreNodeTraversal(const ros::Time &rostime) {
  if (m_publish2DMap) {
...
void OctomapDoseServer::handlePostNodeTraversal(const ros::Time &rostime) {
  if (m_publish2DMap)
    m_mapPub.publish(m_gridmap);
}

void OctomapDoseServer::handleOccupiedNode(const OcTreeT::iterator &it) {
  if (m_publish2DMap && m_projectCompleteMap)
    update2DMap(it, true);
}

void OctomapDoseServer::handleFreeNode(const OcTreeT::iterator &it) {
  if (m_publish2DMap && m_projectCompleteMap)
    update2DMap(it, false);
}

void OctomapDoseServer::handleOccupiedNodeInBBX(const OcTreeT::iterator &it) {
  if (m_publish2DMap && !m_projectCompleteMap)
    update2DMap(it, true);
}

void OctomapDoseServer::handleFreeNodeInBBX(const OcTreeT::iterator &it) {
  if (m_publish2DMap && !m_projectCompleteMap)
    update2DMap(it, false);
}

void OctomapDoseServer::update2DMap(const OcTreeT::iterator &it, bool occupied) {
...
bool OctomapDoseServer::isSpeckleNode(const OcTreeKey &nKey) const {
  OcTreeKey key;
...
void OctomapDoseServer::reconfigureCallback(octomap_server::OctomapServerConfig &config, uint32_t level) {
...
void OctomapDoseServer::adjustMapData(nav_msgs::OccupancyGrid &map, const nav_msgs::MapMetaData &oldMapInfo) const {
...
std_msgs::ColorRGBA OctomapDoseServer::heightMapColor(double h) {
